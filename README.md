...
3. 打开目标应用：
   ...
   - 如果无提示，则默认目录为 `/[内部存储]/DCIM/Camera1/`。目录不存在请手动创建。
   - **提示**：私有目录下的 `Camera1` 仅对当前应用生效。

## 准备替换素材
1. 在目标应用打开相机预览时，气泡消息会显示分辨率（示例：“宽：... 高：...”）。根据该分辨率制作替换视频，命名为 `virtual.mp4`，放到 `Camera1` 目录。若无提示，可直接使用原视频分辨率。
2. 如果拍照仍显示真实照片且收到 `发现拍照` 提示及分辨率，请按提示分辨率准备一张图片，命名为 `1000.bmp`，放入 `Camera1` 目录（其他格式更名为 .bmp 亦可）。若无提示，则无需该文件。
3. 可选开关（全局实时生效，均放在 `/[内部存储]/DCIM/Camera1/`）：
